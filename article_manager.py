...
class ArticleManager:
    """Manages structured storage of top priority articles"""
    
    def __init__(self, storage_file: str = "data/articles.json"):
        self.storage_file = storage_file
        self.articles = self._load_articles()
    
    def _load_articles(self) -> Dict:
        """Load existing articles from JSON storage"""
    ...
    def _save_articles(self):
        """Save articles to JSON storage"""
        os.makedirs(os.path.dirname(self.storage_file), exist_ok=True)
        
        with open(self.storage_file, 'w', encoding='utf-8') as f:
            json.dump(self.articles, f, ensure_ascii=False, indent=2)
        
        print(f"💾 Saved articles to {self.storage_file}")
    ...
    def article_exists(self, title: str, url: str) -> bool:
        ...
        for date_data in self.articles.values():
            for article in date_data.get('articles', []):
                if article.get('hash') == article_hash:
                    return True
        return False
    
    def add_articles(self, date_str: str, articles: List[Dict], source: str = "CoinDesk"):
        ...
        """
        if date_str not in self.articles:
            self.articles[date_str] = {
                'date': date_str,
                'source': source,
        ...
        for article in articles:
            ...
            self.articles[date_str]['articles'].append(article_data)
            new_articles += 1
    ...
    def get_articles(self, date_str: str) -> List[Dict]:
        ...
        return self.articles.get(date_str, {}).get('articles', [])
    
    def get_latest_articles(self, count: int = 3) -> List[Dict]:
        """Get the most recent articles across all dates
    ...
    def cleanup_old_articles(self, days_to_keep: int = 30):
        ...
        dates_to_remove = [date for date in self.articles.keys() if date < cutoff_date]
        
        for date in dates_to_remove:
            del self.articles[date]
        
        if dates_to_remove:
            self._save_articles()
            print(f"🗑️  Cleaned up articles for {len(dates_to_remove)} old dates")
    
    def get_stats(self) -> Dict:
        """Get statistics about stored articles"""
        total_articles = sum(len(data.get('articles', [])) for data in self.articles.values())
        dates_count = len(self.articles)
        
        return {
            'total_articles': total_articles,
            'total_dates': dates_count,
            'dates': sorted(self.articles.keys())
        }
    ...
    def export_to_markdown(self, date_str: str, output_file: str):
        ...
        date_data = self.articles[date_str]
